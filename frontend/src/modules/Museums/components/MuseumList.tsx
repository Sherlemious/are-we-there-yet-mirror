...
interface MuseumListProps {
  ...
  role: 'guest' | 'tourist'; // Define user roles
    ...
    <div className={customStyles.container}>
      ...
      <div className={customStyles.filterContainer}>
        ...
        <div className={customStyles.priceFilters}>
          <input
            type="number"
            placeholder="Min Price Native"
            value={minPriceNative}
            onChange={(e) => setMinPriceNative(Number(e.target.value) || '')}
            className={customStyles.priceInput}
          />
          <input
            type="number"
            placeholder="Max Price Native"
            value={maxPriceNative}
            onChange={(e) => setMaxPriceNative(Number(e.target.value) || '')}
            className={customStyles.priceInput}
          />
        </div>
        <div className={customStyles.priceFilters}>
          <input
            type="number"
            placeholder="Min Price Student"
            value={minPriceStudent}
            onChange={(e) => setMinPriceStudent(Number(e.target.value) || '')}
            className={customStyles.priceInput}
          />
          <input
            type="number"
            placeholder="Max Price"
            value={maxPriceStudent}
            onChange={(e) => setMaxPriceStudent(Number(e.target.value) || '')}
            className={customStyles.priceInput}
          />
        </div>
      </div>
            ...
            {filteredMuseums.map((museum, index) => (
              <div key={index} className={customStyles.slide}>
                <div className={customStyles.slideContent}>
                  ...
                  <p className={customStyles.slideText}>{museum.category}</p>
                  <p className={customStyles.slideText}>Opening Hours: {museum.opening_hours}</p>
                  <p className={customStyles.slideText}>Ticket Prices:</p>
